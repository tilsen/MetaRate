...
% steps can be skipped, and instead, the processed data can be used,
% available here: https://osf.io/df5ub/?view_only=f7786a8452674dc6b621d661a5d2d9e6
...
metarate_helpers;
%% 
% NOTE: The following paths in |metarate_helpers.m| must be edited for 
% the user's system:
%% 
% * h.corpus_dir (directory of Haskins IEEE corpus)
% * h.data_dir     (directory to store processed data )

if ~exist(h.corpus_dir,'dir') 
    fprintf('warning: initial data extraction steps requires the Haskins IEEE corpus\n'); end
if ~exist(h.data_dir,'dir')
    fprintf('error: metarate data directory is required for this script\n'); return; end
...
%% Target data tables
% Make separate data tables for vowel and consonant durations. 
% 
...
%%
% Conducts analyses with full-utterance windows. Saves output in
% |metarate_corr_fullutt.mat| in data directory

metarate_utterance_windows;
%% 
